...
const HasilMobil = () => {
    ...
    <Button className="hasilbtn00" onClick={handleClick}>
        ...
        {list.length > 0 ? list.map((car) => (
        <div key={car.id}>
            
            <Card className="hasilcardmobil">
            <img width={333} height={160} src={car.image || PLACEHOLEDER_URL} alt={car.name} />
            
            <Card.Body>
            <Card.Title className="hasilcarname">{car.name}</Card.Title>
            <Card.Text className="hasilcarprice">{formatCurrency(car.price)} / hari</Card.Text>
            ...
            <Button className="detailtombol"><Link to={`DetailMobil/${car.id}`}>
                Pilih Mobil
            </Link>
            </Button>
